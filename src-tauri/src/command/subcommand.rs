...
use crate::{
    ...
    cli, cli_unknown_arg, init_statics, Cli, ARK_SHELF_FOLDER, ARK_SHELF_WORKING_DIR,
    METADATA_PATH, PREVIEWS_PATH, PROPERTIES_STORAGE_FOLDER, SCORES_PATH,
};

use tauri::{api::cli::SubcommandMatches, Builder, Runtime};
use url::Url;
...
pub async fn process_subcommand(sub: Box<SubcommandMatches>, app: &tauri::App) {
...
async fn create_link(url: String, properties: arklib::link::Properties) -> Result<String> {
    crate::create_link(&url, properties).await
}
...
async fn delete_link(name: String) -> Result<()> {
    ...
    let properties_path = PROPERTIES_STORAGE_FOLDER.get().unwrap().join(&name);
    fs::remove_dir_all(properties_path).unwrap();
...
#[tauri::command]
async fn generate_link_preview(link: arklib::link::Link) -> Result<OpenGraph> {
    link.get_preview().await.map_err(|_| CommandError::IO)
}

/// Get the score list
#[tauri::command]
pub async fn get_scores(scores: tauri::State<'_, Arc<Mutex<Scores>>>) -> Result<Scores> {
    let ark = ARK_SHELF_WORKING_DIR
        .get()
        .unwrap()
...
async fn read_link(name: String) -> Result<LinkWrapper> {
    let path_buf = dirs::home_dir()
        ...
        .unwrap();
    let working_dir = ARK_SHELF_WORKING_DIR.get_or_init(|| path_buf.clone());
    let meta = fs::metadata(&working_dir.join(name.clone()).clone())?;
    let created_time = match meta.created() {
        Ok(time) => Some(time),
        Err(_) => None,
    };
    let link =
        Link::load(working_dir, &PathBuf::from(name.clone())).map_err(|_| CommandError::Arklib)?;
    Ok(LinkWrapper {
...
#[tauri::command]
fn gui_state(state: tauri::State<'_, Arc<Mutex<GuiState>>>) -> Result<bool> {
    Ok(state.lock().unwrap().gui_once)
}
...
fn get_fs_links() -> Vec<DirEntry> {
    let ark = ARK_SHELF_WORKING_DIR.get().unwrap();
    WalkDir::new(ark)
        .max_depth(1)
        .into_iter()
        .filter(|file| {
